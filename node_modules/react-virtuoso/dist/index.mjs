...
const Viewport$2 = ({ children }) => {
  ...
  const alignToBottom = useEmitterValue$2("alignToBottom");
  const viewportRef = useSize(React.useMemo(() => compose(viewportHeight, (el) => correctItemSize(el, "height")), [viewportHeight]));
  React.useEffect(() => {
    if (ctx) {
...
const TableRoot = /* @__PURE__ */ React.memo(function TableVirtuosoRoot(props) {
  ...
  const context = useEmitterValue("context");
  const theadRef = useSize(React.useMemo(() => compose(fixedHeaderHeight, (el) => correctItemSize(el, "height")), [fixedHeaderHeight]));
  const tfootRef = useSize(React.useMemo(() => compose(fixedFooterHeight, (el) => correctItemSize(el, "height")), [fixedFooterHeight]));
