...
def delete_existing_vector_store_file(vector_store_id, file_id):
    ...
    try:
        ...
        logging.info(f"Retrieved files from vector store {vector_store_id}")
...
def upload_to_vector_store(file_path):
    ...
    # Retrieve the file ID using the filename
    file_id = get_file_id_by_filename(os.path.basename(file_path))
    if file_id:
        # Delete the existing file in the vector store by file ID before uploading the new one
        delete_existing_vector_store_file(VECTOR_STORE_ID, file_id)
